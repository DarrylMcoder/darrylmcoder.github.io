var unblocker = function(config) {
  function unblock(event) {
    try {
      ...
      var url = request.url;
      var headers = _.cloneDeep(event.request.headers);
    ...
    .then(res => {
      ...
      var response = _.cloneDeep(res);
      var headers = _.cloneDeep(res.headers);
      ...
      config.responseMiddleware.forEach(middleware => {
        middleware(data);
      });
